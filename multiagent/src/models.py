...
Agent IDs are dynamic strings (not Literal) so that any scenario's
agents can be routed without code changes.
"""
...
class RoutingDecision(BaseModel):
    ...
    primary_agent: str = Field(
        ..., description="The agent ID that should respond"
    )
    secondary_agent: str = Field(
        default="none", description="Secondary agent ID, or 'none'"
    )
    topic: str = Field(
    ...
    tools_allowed: bool = Field(
        ...
        description=(
            ...
            "or check real-time data. False for explanations, introductions, opinions."
        ),
    )
...
class ProjectPlan(BaseModel):
    ...
    def summary(self) -> str:
        if not any([self.notes, self.blockers, self.project_name]):
